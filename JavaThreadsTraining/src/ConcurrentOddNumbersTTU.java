...
public class ConcurrentOddNumbersTTU {
...
// START BOILER PLATE FORK_JOIN CONCURRENT CODE
	...
	private static void createRunnables() {
		...
		for (int aThreadIndex = 0; aThreadIndex < NUM_THREADS; aThreadIndex++) {
			...
			workers[aThreadIndex] = new OddNumbersWorkerTTU(randomNumbers, aStartIndex, aStopIndex);
 ...
 *
 */
class OddNumbersWorkerTTU implements Runnable {
	int[] numbers;
	int startIndex, stopIndex;

	public OddNumbersWorkerTTU(int[] aNumbers, int aStartIndex, int aStopIndex) {
		numbers = aNumbers;
	...
	public void run() { // executed when we start the associated thread.
		...
		ConcurrentOddNumbersTTU.fillOddNumbers(numbers, startIndex, stopIndex);
	}
